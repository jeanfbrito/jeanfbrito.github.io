---
name: blog-preview
description: "Start the local Jekyll preview for jeanfbrito.github.io with drafts visible when a blog preview is requested."
allowed-tools: Bash
---
...
Use the **Bash tool with `run_in_background: true`** — do NOT run in the foreground. This is the Claude Code equivalent of `terminal(background=true)`.

```bash
cd "$BLOG_REPO" && BUNDLE_PATH=vendor/bundle bundle exec jekyll s --drafts --host 0.0.0.0 --port 4001
```

Run this with `run_in_background: true` so Jekyll keeps serving without blocking the conversation.
...
Adapted for Claude Code from the Hermes Agent skill at `skills/hermes/blog-preview/SKILL.md`. Substance is the same; only paths, platform commands, and tool names changed.
