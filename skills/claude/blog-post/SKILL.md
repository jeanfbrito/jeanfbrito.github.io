...
### Liquid in code fences (blanks the page)

Jekyll runs Liquid over the whole file before Markdown, including fenced code. Any `{{ ... }}`
or `{% ... %}` in a snippet — GitHub Actions `${{ matrix.os }}`, Helm/Go templates, Jinja —
is evaluated, not displayed, and an unknown tag can render the post as an empty page while the
server still returns `200`. Wrap such fences by placing the raw tags on their own lines around the whole fence:
...
Before reporting, `grep -n '{{\|{%' "$DRAFT"` and confirm every hit is inside a raw block or is
a deliberate Liquid tag such as `{% post_url %}`.
...
6. **Start the preview server and hand back the draft URL.** Do not just print the command;
   ...
   then confirm the draft itself renders. Check the **body size, not just the status code**: a
   Jekyll server that has been running for days can emit a 0-byte file for a new draft and
   still answer `200` (seen 2026-09-21 with a server started 13 days earlier). `404` means
   Jekyll skipped the post, usually a future date or a front-matter error.

   ```bash
   curl -s -o /dev/null -w "%{http_code} %{size_download}" "http://localhost:4001/posts/<slug>/"
   ```

   Anything under ~5000 bytes is a blank page. Fix it by killing the old server
   (`lsof -tiTCP:4001 -sTCP:LISTEN | xargs kill`) and starting a fresh one; then re-check.
   Also grep the draft for `{{` outside a `{% raw %}` block first — Liquid parses `${{ matrix.x }}`
   even inside code fences and can blank the page (see Liquid rule under the style guide).
